...
![](https://shiny.rstudio.com/py/api/_static/shiny-hex.svg){width=100px}  

[Use Rshiny App of this EDA!!](https://haitieliu.shinyapps.io/Project2/) 
...
# plot Job Satisfaction by OverTime
data1_summary2 <- data1 %>%
  group_by(OverTime) %>%
  summarize(mean_JobSatisfaction = mean(as.numeric(JobSatisfaction)))

ggplot(data1_summary2, aes(x = OverTime, y = mean_JobSatisfaction, fill = OverTime)) + 
  geom_bar(stat = "identity") +
  labs(title = "Job Satisfaction by Overtime Status",
       subtitle = "Comparison of Job Satisfaction Levels by Overtime Status",
       x = "Overtime Status",
       y = "Mean Job Satisfaction",
       fill = "Overtime Status") +
  ggthemes::theme_clean() +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust = 1),
        panel.spacing.x = unit(2, "cm")) +
  scale_fill_discrete(name = "Overtime Status") +
...
for (i in 1:iterations) {
  sample_rows <- sample(dim(data2)[1], dim(data2)[1] * split)
...
ggplot(nb_df_mean, aes(x = threshold, y = mean)) +
  geom_line() +
  labs(x = "Threshold", y = "Average AUC")
